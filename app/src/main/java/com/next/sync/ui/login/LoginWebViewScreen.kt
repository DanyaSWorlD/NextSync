...
@Composable
fun LoginWebViewScreen(loginState: LoginState, navigate: (String) -> Unit) {
    ...
    AndroidView(factory = {
        WebView(it).apply {
            ...
            this.webViewClient = CustomWebViewClient(loginState) {
...
class CustomWebViewClient(
    ...
    private val loggedInCallback: () -> Unit
) : WebViewClient() {
    ...
    override fun shouldOverrideUrlLoading(view: WebView?, url: String?): Boolean {
        ...
        if (url.startsWith(prefix)) {
            ...
            for (pair in data.split('&')) {
                ...
                if (field[0] == "server")
                    loginState = loginState.copy(serverAddress = field[1])
                if (field[0] == "user")
